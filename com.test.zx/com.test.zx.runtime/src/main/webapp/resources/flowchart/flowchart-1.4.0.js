...
(function() {
  ...
  FlowChart.prototype.handleRoute = function(source, target) {
	this.routes.push({"source": source, "target": target});
  };
  ...
  FlowChart.prototype.render = function() {
    ...
    for (i = 0, len = this.symbols.length; i < len; i++) {
      ...
      if (symbol.x + symbol.width > paperWidth) {
    	  paperWidth = symbol.x + symbol.width
      }
      if (symbol.y + symbol.height > paperHeight) {
    	  paperHeight = symbol.y + symbol.height
      }
    }
    ...
    for (i = 0, len = this.symbols.length; i < len; i++) {
      ...
      if (symbol.x > 0 || symbol.y > 0) {
    	  symbol.shiftX(symbol.x);
    	  symbol.shiftY(symbol.y);
    	  symbol.isPositioned = true;
      }
    }
...
//    }
    ...
    for (i = 0, len = this.routes.length; i < len; i++) {
    	...
    	route.source.drawLineTo(route.target);
    }
    ...
    this.paper.setSize((paperWidth * scale) + (maxWidth * scale), (paperHeight * scale) + (lineWidth * scale));
  ...
  function Symbol(chart, options, symbol) {
    ...
    if (options.link) { 
//    	if (options.link.indexOf('javascript:') >= 0) {
//    	  this.text.attr('onclick', options.link.replace('javascript:', ''));
//    	} else {
...
//    	}
    ...
    if (symbol) {
      ...
      //by TQ 支持自定义size
      if(symbol.attr('width') == 0 || symbol.attr('height') == 0) {
      	symbol.attr({
      		...
      		this.text.attr({
      			'x': (symbol.attr('width') - this.text.getBBox().width) / 2
      		});
      }
  ...
  Symbol.prototype.drawLineTo = function(symbol, text, origin) {
	    ...
	    if (!origin) {
	    	if (isUpper) {
	    		origin = this.symbolType  === 'gateway' ? 'top' : 'right';
	    	} else if (isOnSameLine) {
	    		origin = 'right';
	    	} else {
	    		origin = this.symbolType  === 'gateway' ? 'bottom' : 'right';
	    	}
	    }
	  
	    if (origin === 'top') {
	    	if (isOnSameColumn) {
	    		line = drawLine(this.chart, top, symbolBottom, text);
		        this.topStart = true;
		        symbol.bottomEnd = true;
		        maxX = top.x;
	    	} else if (isRight) {
	    		line = drawLine(this.chart, top, [
   		          {x: top.x, y: symbolLeft.y},
   		          {x: symbolLeft.x, y: symbolLeft.y}
   		        ], text);
   		        this.topStart = true;
   		        //symbol.topEnd = true;
   		        symbol.leftEnd = true;
   		        maxX = top.x + (top.x - symbolTop.x)/2;
	    	}
	    	...
	    	if (isOnSameColumn) {
	    		...
	    		line = drawLine(this.chart, bottom, [
		          //{x: bottom.x, y: bottom.y + (symbolTop.y - bottom.y) / 2},
		          //{x: symbolTop.x, y: bottom.y + (symbolTop.y - bottom.y) / 2},
		          {x: bottom.x, y: symbolLeft.y},
		          {x: symbolLeft.x, y: symbolLeft.y}
		        ], text);
		        this.bottomStart = true;
		        //symbol.topEnd = true;
	    	...
	    	if (isOnSameLine) { //默认right,left的话不支持
	    		...
	    		line = drawLine(this.chart, right, [
  		          {x: symbolBottom.x, y: right.y},
  		          {x: symbolBottom.x, y: symbolBottom.y},
  		        ], text);
	    		symbol.bottomEnd = true;
	    	} else if (isUnder) {
	    		line = drawLine(this.chart, right, [
  ...
  function Task(chart, options) {
	var img = 'task.jpg';
	if (options.flowstate != 'future') {
		img = 'task_' + options.flowstate + '.jpg';
	}
  	var symbol = chart.paper.image('app/procmonitor/' + img, 0, 0, options.width || 0, options.height || 0);
  	options = options || {};
  	Symbol.call(this, chart, options, symbol);
  }
  f.inherits(Task, Symbol);
  function Gateway(chart, options) {
  	var symbol = chart.paper.image('app/procmonitor/gateway.jpg', 0, 0, options.width || 0, options.height || 0);
  	options = options || {};
  	options.text = '';
  	Symbol.call(this, chart, options, symbol);
  }
  f.inherits(Gateway, Symbol);
  ...
  function parse(input) {
    ...
    var chart = {
      ...
      drawSVG: function(container, options) {
        ...
        function getDisplaySymbol(s) {
          ...
          switch (s.symbolType) {
            ...
            case 'end':
              ...
              break;
            case 'task':
            	dispSymbols[s.key] = new Task(diagram, s);
            	break;
            case 'gateway':
            	dispSymbols[s.key] = new Gateway(diagram, s);
            	break;
            default:
        ...
        (function constructChart(symbols, routes) {
        	for (var s in symbols) {
        		diagram.handle(getDisplaySymbol(symbols[s]));
        	}
        	for (var i=0, n=routes.length; i<n; i++) {
        		var route = routes[i];
        		var source = getDisplaySymbol(route.source);
        		var target = getDisplaySymbol(route.target);
        		diagram.handleRoute(source, target);
        	}
        })(this.symbols, this.routes);
    ...
    while (lines.length > 0) {
      ...
      if (line.indexOf('{') == 0) { //activity
      	var symbol = JSON.parse(line);
      	chart.symbols[symbol.key] = symbol;
      } else if (line.indexOf('->') >= 0) {//route
    	var flowSymbols = line.split('->');
    	for (var  i= 0, lenS = flowSymbols.length; i < lenS; i++) {
    		if (i + 1 <lenS) {
    			var source = getSymbol(flowSymbols[i]);
    			var target = getSymbol(flowSymbols[i + 1]);
    			chart.routes.push({"source" : source, "target" : target});
    		}
    	}
      }
